    ...
    <Card title='Departmental Resources' className="shadow-md m-2">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
        {resources.map((resource) => (
          <Card key={resource.id} title={resource.title} className="shadow-md">
            ...
            <div className="mt-2">
              ...
              <Button
                className="mt-2"
                onClick={() => {
                  dispatch(addComment({ id: resource.id, comment: { user: "Student", text: commentText } }));
                  setCommentText("");
                }}
              >
                Comment
            ...
            <div className="mt-2">
              {(resource.comments || []).map((comment, index) => (
                <p key={index} className="text-sm italic">💬 {comment.user}: {comment.text}</p>
              ))}
            </div>
          </Card>
        ))}
